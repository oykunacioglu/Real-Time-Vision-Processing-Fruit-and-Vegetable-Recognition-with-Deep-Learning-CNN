...
👁️ Real-Time Vision Processing: Fruit & Vegetable Recognition
This project is an AI-powered object detection and classification system via webcam using Convolutional Neural Networks (CNN) and Computer Vision. By utilizing the Fruit-360 dataset, the model has been trained and stabilized to perform reliably under real-world conditions.
...
A 280x280 pixel focus area was created to keep the model's focus on the center of the frame and eliminate background gnoise.

🛑 Confidence Thresholding
A 70% confidence threshold was applied to prevent random guesses; the system only presents results to the user when it is statistically certain.

🗺️ Heuristic Label Mapping
A name_fixer dictionary was developed to synchronize the model's internal indices with the local file system (e.g., mapping "Zucchini 1" to "Green Apple").
